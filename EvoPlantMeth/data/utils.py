...
def get_output_names(data_file, *args, **kwargs):
    return hdf.ls(data_file, 'outputs', recursive=True, groups=False, *args, **kwargs)


def get_replicate_names(data_file, *args, **kwargs):
    return hdf.ls(data_file, 'inputs/cpg', recursive=False, groups=True, must_exist=False, *args, **kwargs)


def get_anno_names(data_file, *args, **kwargs):
    return hdf.ls(data_file, 'inputs/annos', recursive=False, *args, **kwargs)
...
def read_cpg_profile(filename, chromos=None, nb_sample=None, round=False, sort=True, nb_sample_chromo=None):
    if is_bedgraph(filename):
        usecols, skiprows = [0, 1, 3], 1
    else:
        usecols, skiprows = [0, 1, 2], 0
        
    dtype = {usecols[0]: str, usecols[1]: np.int32, usecols[2]: np.float32}
    nrows = nb_sample if chromos is None and nb_sample_chromo is None else None
    ...
    if chromos is not None:
        chromos = [str(c) for c in chromos] if not isinstance(chromos, list) else chromos
        d = d.loc[d.chromo.isin(chromos)]
        if len(d) == 0: raise ValueError('No data available for selected chromosomes!')
...
class GzipFile(object):
    def __init__(self, filename, mode='r', *args, **kwargs):
        ...
        self.fh = gzip.open(filename, mode, *args, **kwargs) if self.is_gzip else open(filename, mode, *args, **kwargs)

    def __iter__(self): return self.fh.__iter__()
    def __next__(self): return self.fh.__next__()
    def read(self, *args, **kwargs): return self.fh.read(*args, **kwargs)
    def readline(self, *args, **kwargs): return self.fh.readline(*args, **kwargs)
    def readlines(self, *args, **kwargs): return self.fh.readlines(*args, **kwargs)
    def write(self, data):
        if self.is_gzip and isinstance(data, str): data = data.encode()
        self.fh.write(data)
    def writelines(self, *args, **kwargs): self.fh.writelines(*args, **kwargs)
    def tell(self, *args, **kwargs): return self.fh.tell(*args, **kwargs)
    def seek(self, *args, **kwargs): self.fh.seek(*args, **kwargs)
    def closed(self): return self.fh.closed
    def close(self): self.fh.close()
    def iter(self): return self.fh.__iter__()
