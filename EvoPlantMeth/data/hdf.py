...
def ls(filename, group='/', recursive=False, groups=False, regex=None, nb_key=None, must_exist=True):
...
def reader(data_files, names, batch_size=128, nb_sample=None, shuffle=False, loop=False):
    ...
    with h5.File(data_files[0], 'r') as h5_file:
        for name in names:
    ...
    if nb_sample:
        ...
        for data_file in data_files:
            with h5.File(data_file, 'r') as h5_file:
    ...
    while True:
        ...
        with h5.File(data_files[file_idx], 'r') as h5_file:
            data_file = {name: h5_file[name] for name in names}
            ...
            for batch in range(nb_batch):
                ...
                yield {name: data_file[name][batch_start:batch_end] for name in names}
...
def read_from(data_reader, nb_sample=None):
...
def read(data_files, names, nb_sample=None, batch_size=1024, *args, **kwargs):
    data_reader = reader(data_files, names, batch_size=batch_size, nb_sample=nb_sample, loop=False, *args, **kwargs)
