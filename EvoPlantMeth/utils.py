...
def linear_weights(length, start=0.1):
    ...
    tmp = weights
    if length % 2: tmp = tmp[:-1]
    return np.hstack((weights, tmp[::-1]))
...
def get_from_module(identifier, module_params, ignore_case=True):
    if ignore_case:
        _module_params = {k.lower(): v for k, v in six.iteritems(module_params)}
...
def format_table_row(values, widths=None, sep=' | '):
    ...
    return sep.join([str(v) if v is not None else '' for v in values])

def format_table(table, colwidth=None, precision=2, header=True, sep=' | '):
    col_names = list(table.keys())
    if not isinstance(precision, list): precision = [precision] * len(col_names)
    col_widths, tot_width, nb_row = [], 0, None
    ftable = OrderedDict()
    ...
    for col_idx, col_name in enumerate(col_names):
        ...
        for value in table[col_name]:
            if value is None: value = ''
            elif isinstance(value, float): value = '{0:.{1}f}'.format(value, precision[col_idx])
            else: value = str(value)
            width = max(width, len(value))
            values.append(value)
        ftable[col_name] = values
        col_widths.append(width)
        nb_row = len(values) if not nb_row else max(nb_row, len(values))
        tot_width += width
    ...
    for row in range(nb_row):
        values = [col_values[row] if row < len(col_values) else None for col_values in six.itervalues(ftable)]
        rows.append(format_table_row(values, col_widths, sep=sep))
    return '\n'.join(rows)

def filter_regex(values, regexs):
    if not isinstance(values, list): values = [values]
    if not isinstance(regexs, list): regexs = [regexs]
    filtered = set()
    for value in values:
        for regex in regexs:
            if re.search(regex, value): filtered.add(value)
...
class ProgressBar(object):
    ...
    def update(self, amount):
        ...
        if tricker:
            ...
            msg = '{value:{nb_digit}d}/{nb_tot:d} ({per:3.1f}%)'.format(
                value=self._value, nb_digit=nb_digit, 
                nb_tot=self.nb_tot, per=self._value / self.nb_tot * 100)
            self.logger(msg)
            self._nb_interval = 0
